...
- Реализован `ApiKeyService` с методами `validate()` (проверка хэша через bcrypt, статус, срок действия), `findById()`, `revoke()`.
- Обновлён `ApiKeyGuard` для использования сервиса вместо заглушки: проверка ключа из БД, сохранение `request.apiKeyEntity` для использования в контроллерах.
- Обновлён `AuthModule` с подключением TypeORM репозитория и экспортом сервиса.
- Добавлены unit-тесты для `ApiKeyService` и `ApiKeyGuard` (15 тестов, все проходят).

---

## День 3 — статус

- Установлены зависимости `@nestjs/throttler` и `ioredis` для rate limiting.
- Реализован `RedisThrottleStorage` — кастомный storage для ThrottlerModule с использованием Redis (атомарные операции через multi, TTL управление).
- Создан `ApiKeyThrottlerGuard` — кастомный guard, который использует `rate_limit_per_min` из `apiKeyEntity` для динамических лимитов (вместо фиксированного лимита).
- Настроен `ThrottlerModule` в `AppModule` с Redis storage и глобальным guard.
- Добавлены unit-тесты для `ApiKeyThrottlerGuard` (все проходят). Тесты для `RedisThrottleStorage` требуют доработки моков (можно использовать интеграционные тесты с реальным Redis).
